#!/usr/bin/env python3
"""
Build blog post for Section 122 Scenario Analysis in the GTA API database.
...
generated blog HTML from results/scotus/s122/blog/.

Publication status is set to 0 (unpublished) until manually activated.

Usage:
    python3 build_s122_blog_post.py --dry-run    # Preview changes
    python3 build_s122_blog_post.py               # Apply to database
...
# Report metadata
REPORT_TITLE = "Section 122 invoked: two interpretations of the new US tariff regime"
REPORT_SLUG = "S122-Tariff-Scenario-Analysis"
REPORT_SUBTITLE = (
    "After SCOTUS struck down IEEPA tariffs, President Trump invoked Section 122. "
    "The trade-weighted average US tariff rate lands at either 18.3% (flat surcharge) or 13.7% (IEEPA-like stacking)."
)
REPORT_TYPE_ID = 1  # Report
PUBLICATION_DATE = datetime(2026, 2, 20, 14, 0, 0)
...
ABSTRACT = (
    "The Supreme Court ruled that IEEPA does not authorise presidential tariffs. "
    "President Trump has invoked Section 122 of the Trade Act of 1974 in response. "
    "The trade-weighted average US tariff rate now depends on how the Section 122 surcharge "
    "interacts with remaining tariffs: a flat 10% surcharge yields 18.3%, while an IEEPA-like "
    "stacking interpretation yields 13.7%. The analysis covers 274,000+ trade flows at the "
...
BLOG_HTML_PATH = PROJECT_ROOT / "results" / "scotus" / "s122" / "blog" / "s122_blog_body.html"
...
def main():
    parser = argparse.ArgumentParser(
        description="Build Section 122 scenario analysis blog post in GTA database"
    )
    parser.add_argument(
    ...
    if BLOG_HTML_PATH.exists():
        ...
        print("Run generate_s122_output.R first to generate the blog HTML.")
        sys.exit(1)
